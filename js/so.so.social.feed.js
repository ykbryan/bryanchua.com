				...
				success: function(res) {
					...
					if (res.query.results != null) {
						jQuery.each(res.query.results.rss.channel.item, function(i, o) {
							...
							if (res.query.results.rss.channel.link != undefined && res.query.results.rss.channel.link.indexOf('github') > -1) { desc = desc  + ' <a href="' + o.link + '" target=\"_blank\">on Github</a>';}
							if (o.link.indexOf("dribbble.com") > -1) { desc = 'Shot <a href="' + o.link + '" target=\"_blank\">' + desc + '</a> over at Dribble'; }
							...
							var html = '<li style="background: url(' + bg + ') no-repeat top left;">' + desc + '<br /><p style="text-align:left; font-size:12px; padding-bottom:10px">' + obj.relative_time(o.pubDate.replace(/\,/g,'')) + ' (<a href="' + o.link + '" target=\"_blank\">go to site</a>)</p></li>';
